...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  public void setArchiveStateRoot(byte[] root) {
    this.block = this.block.toBuilder().setBlockHeader(
        this.block.getBlockHeader().toBuilder()
            .setArchiveStateRoot(ByteString.copyFrom(root))).build();
  }
  ...
  public Sha256Hash getArchiveStateRoot() {
    ByteString archiveStateRoot = this.block.getBlockHeader().getArchiveStateRoot();
    if (archiveStateRoot != null && !archiveStateRoot.isEmpty()) {
      return Sha256Hash.wrap(archiveStateRoot);
    }
    return Sha256Hash.ZERO_HASH;
  }
  ...
  public String toString() {
    ...
    toStringBuff.append("state root=").append(getArchiveStateRoot()).append("\n");
